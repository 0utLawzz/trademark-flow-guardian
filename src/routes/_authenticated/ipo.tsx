...
export const Route = createFileRoute("/_authenticated/ipo")({
  head: () => ({ meta: [{ title: "IPO — Trademark Management" }] }),
  component: IpoPage,
});
...
function IpoPage() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["ipo_entries"],
    queryFn: async () => {
      const { data, error } = await supabase.from("ipo_entries").select("*").order("entry_date", { ascending: false, nullsFirst: false });
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("ipo_entries").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["ipo_entries"] }); },
  });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-display text-2xl font-semibold">IPO</h1>
          <p className="text-sm text-muted-foreground">Manually-recorded IPO entries. Rows in <Bell className="inline h-3.5 w-3.5 text-accent-foreground align-text-bottom" /> match one of our active trademarks.</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button size="sm"><Plus className="h-4 w-4 mr-1" /> Add entry</Button></DialogTrigger>
          <IpoDialog onDone={() => { setOpen(false); qc.invalidateQueries({ queryKey: ["ipo_entries"] }); }} />
        </Dialog>
      </div>
      <Card>
        ...
        <CardContent>
          {isLoading ? <Loader2 className="h-5 w-5 animate-spin mx-auto my-10 text-muted-foreground" /> : rows.length === 0 ? (
            <p className="py-10 text-center text-sm text-muted-foreground">No IPO entries yet.</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
...
function IpoDialog({ onDone }: { onDone: () => void }) {
  ...
  return (
    <DialogContent className="sm:max-w-md">
      <DialogHeader><DialogTitle>New IPO entry</DialogTitle></DialogHeader>
      <div className="grid gap-3">
        ...
        <div className="space-y-1.5"><Label>Status</Label><Input value={status} onChange={(e) => setStatus(e.target.value)} /></div>
